import { UIButton, UIPanel, UIBreak, UIText } from '../../../../drawUI/ui.js';

import { UIBoolean, UIOutliner } from '../../../ui/ui.three.js';
...
function SidebarSettingsHistory( editor ) {
	...
	const historyTracker = new UIOutliner( editor );

	historyTracker.onChange( function () {

		ignoreObjectSelectedSignal = true;

		editor.history.goToState( parseInt( historyTracker.getValue() ) );

		ignoreObjectSelectedSignal = false;

	} );

	container.add( historyTracker );
	...
	const refreshUI = function () {
		...
		( function addObjects( objects ) {
			...
			for ( let i = 0, l = objects.length; i < l; i ++ ) {
				...
				option.style.opacity = object.undoable !== false ? 1 : 0.5;

				option.style.color = object.undoable !== false ? '' : 'var(--theme-text-light, #95a5a6)';
		...
		historyTracker.setOptions( options );
	...
	signals.historyChanged.add( function ( cmd ) {
		...
		historyTracker.setValue( cmd !== undefined ? cmd.id : null );
