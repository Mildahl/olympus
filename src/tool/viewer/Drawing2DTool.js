...
/**
 * Drawing2DTool - Stateless scene mesh extraction for 2D drawing generation
 *
 * This tool handles THREE.js scene-specific operations (mesh data extraction).
 * All pure drawing logic (cutting, rendering, export) is in the addon's core.js.
 *
 * Architecture:
  ...
  static getMeshDataFromScene(context) {
    ...
    context.editor.scene.traverseVisible((object) => {
      if (object.isMesh && object.geometry && !object.userData?.isHelper) {
        const data = Drawing2DTool.getMeshDataFromObject(object, 0);
